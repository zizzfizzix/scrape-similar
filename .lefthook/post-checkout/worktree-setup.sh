...
if [[ "$1" == "0000000000000000000000000000000000000000" ]]; then
    echo "[lefthook] New worktree detected. Starting setup..."
    ...
    if [ -f "$ORIGIN_REPO/.env" ]; then
        echo "[lefthook] Copying .env from $ORIGIN_REPO"
        cp "$ORIGIN_REPO/.env" .
    else
        echo "[lefthook] Warning: No .env found in origin root ($ORIGIN_REPO)"
    fi

    if [ -f "$ORIGIN_REPO/.env.test" ]; then
        echo "[lefthook] Copying .env.test from $ORIGIN_REPO"
        cp "$ORIGIN_REPO/.env.test" .
    else
        echo "[lefthook] Warning: No .env.test found in origin root ($ORIGIN_REPO)"
    fi

    # 3. Run pnpm install if package.json exists
    if [ -f "package.json" ]; then
        echo "[lefthook] Running pnpm install..."
        # Using --silent to keep worktree creation fast and clean
        pnpm install --silent
    fi

    echo "[lefthook] Setup complete for $(pwd)"
fi
