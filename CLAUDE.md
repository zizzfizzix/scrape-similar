...
- **The exclusion list is itself gated.** It lives in `coverage-exclusions.ts` (imported by `vitest.config.ts`) with the reason for each group, and `src/__tests__/coverage-exclusions.test.ts` asserts: the list matches an explicit literal, so widening it means editing the test in the same diff; every path still exists; every type-only entry really exports no runtime value; and every bootstrap file stays under `BOOTSTRAP_LINE_BUDGET` lines. It shrinks the measured total, so do not add to it to make a check pass.
...
- **Keep new logic out of the bootstrap files**, or it becomes untestable and silently unmeasured: put it in a module beside them and cover it there. The existing examples are `content/bootstrap.ts`, `background/bootstrap.ts` (what the service worker starts), `sidepanel/SidePanelRoot.tsx` and `components/extension-page.tsx` (finding a page's root element and the provider stack the pages share). What is left in each `main.tsx` / `index.ts` is 10-19 lines of `define*` wrapper, log level and provider nesting, and `BOOTSTRAP_LINE_BUDGET` holds them there.
...
- **Unit-test harness**: `tests/support/react.tsx`, hand-rolled on `createRoot` + `act`, not `@testing-library/react`. Settled in #266: the pieces it was missing were `waitFor`, `stubScrolling` and `stubOffsetWidth`, which was less work than rewriting the existing tests and cheaper than two more dependencies in a repo that gates dependency age. It also carries `setInputValue` (React ignores an `input` event whose value it already rendered), `openRadixTrigger` (Radix opens on pointer events jsdom's `click()` never sends) and `findByRole` for portalled content.
