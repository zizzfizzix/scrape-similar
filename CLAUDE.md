...
`eslint.config.ts` is where the conventions below stop being review comments. Four things about it:

- **It is type-aware but sparing with typed rules.** `projectService` is on, so a typed rule is a one-line addition, but only the syntactic sets (`@eslint/js`, `typescript-eslint` recommended, `react-hooks`, `vitest`, `testing-library`, `jest-dom`) are enabled today.
