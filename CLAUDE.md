...
- **`naming-convention` is on for one row of the conventions, not all of them.** Booleans only (#284). The others were measured before being left off, and the counts are recorded beside the rule: casing reports 69 of which 67 are React components the rule has to be told to allow, widening the boolean prefix to properties reports 537 mostly on names the codebase does not own, and the directory row is not something `naming-convention` can see at all. Its two sharp edges are recorded beside it: it strips `null | undefined` from a type before checking (`getNonNullableType()`), so it cannot tell a tri-state from a boolean, and it is scoped to `variable` rather than every declaration.
- **`DEFERRED` is the worklist, not a dumping ground.** Each rule in it is off with the number of violations it reports and why they are not a diff away — `no-floating-promises` (128, of which 65 are `trackEvent`), `no-explicit-any` (74), `testing-library/no-node-access` (265), the four React Compiler rules that want effects restructured (22). Re-enabling one means fixing what it names; the counts are there to be checked, so update them if they move. Never add a rule to `DEFERRED` to make a check pass without recording the same two things.
...
- **A boolean variable reads as a question**: `is`, `has`, `should`, `can`, `was` or `will`, enforced by `@typescript-eslint/naming-convention` (#284). Two things about it are settled rather than open. It covers the `variable` selector only, so a prop or a message-payload field keeps the name its contract gives it and a passthrough reads `showEmptyRows={shouldShowEmptyRows}` — the prop says what the component does with it, the variable says what it holds. And a variable holding a `ConsentState` is named `…consentState` and exempted by that name, because the type is `boolean | undefined` where `undefined` means "not asked yet", and a prefix would promise two states where there are three.
