...
- **`no-misused-promises` runs with every option on, JSX attributes included.** An async `onClick` reads like the idiomatic React spelling, and it would be one if the only alternative were `onClick={() => void handle()}` — the same dropped rejection with more syntax around it. It is not: `onClick={() => fireAndForget(handle())}` reports what the handler threw, and a rejected click handler is exactly as invisible as the service-worker listener this rule is usually credited with catching.
...
- **A promise nobody holds goes through `src/utils/fire-and-forget.ts`.** `fireAndForget(task)` starts an async task from a caller that cannot await it (a synchronous entrypoint, the message router that must return `true` before its dispatcher finishes) and reports the rejection; `asListener(handler)` is the same thing shaped for a callback position that ignores the return value. A JSX handler is the commonest case and takes the same route — `onClick={() => fireAndForget(handleSave())}`, never `onClick={handleSave}` with an async `handleSave`, which drops the rejection on the floor. A bare `void x()` is right only where the callee already logs its own failures and resolves either way — `getConsentState`, `getAllPresets`, `setupUninstallUrl` — and each of those sites says so in a comment. `trackEvent` returns `void` for the same reason: `captureEvent` behind it swallows and logs everything, and only the tests need the promise.
