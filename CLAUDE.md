...
- **jsdom shims** live in `vitest.setup.ts` (`matchMedia`, `PointerEvent`, pointer capture, `scrollIntoView`, `scrollTo`, `ResizeObserver`) — everything Radix and the layout-measuring components reach for. `tests/support/dom.ts` holds the one shim a test has to opt into with a value of its own, `stubOffsetWidth`, and `tests/support/flush-microtasks.ts` holds `flushMicrotasks()` — one macrotask turn, which drains the microtask queue and is what a test needs after triggering a listener that `reportingListener` left unawaited, or after a storage write whose watchers fire on the next turn. Import it; do not re-declare it locally, which is how the suite ended up with eight copies of the same line under three names. It does not advance timers, so a handler waiting on a real one wants `vi.waitFor` instead.
...
`eslint.config.ts` is where the conventions below stop being review comments. Six things about it:

- **It is type-aware, and the typed rules arrive in named groups.** `projectService` is on, so a typed rule is a one-line addition. What is enabled is the syntactic sets (`@eslint/js`, `typescript-eslint` recommended, `react-hooks`, `vitest`, `testing-library`, `jest-dom`) plus three typed groups: `ALREADY_CLEAN`, the rules this codebase already satisfies, so they only ever fail on something new; `PROMISE_RULES`, the pair type-aware linting was switched on for ([#277](https://github.com/zizzfizzix/scrape-similar/issues/277)); and the boolean-naming rule below. Everything else typed is a backlog rather than a decision — see #282 for the ladder.
...
- **`no-misused-promises` runs with every option on, JSX attributes included.** An async `onClick` reads like the idiomatic React spelling, and it would be one if the only alternative were `onClick={() => void handle()}` — the same dropped rejection with more syntax around it. It is not: `onClick={() => reportRejection(handle())}` reports what the handler threw, and a rejected click handler is exactly as invisible as the service-worker listener this rule is usually credited with catching.
...
- **A promise nobody holds goes through `src/utils/report-rejection.ts`.** `reportRejection(task)` starts an async task from a caller that cannot await it (a synchronous entrypoint, the message router that must return `true` before its dispatcher finishes) and reports the rejection; `reportingListener(handler)` is the same thing shaped for a callback position that ignores the return value. A JSX handler is the commonest case and takes the same route — `onClick={() => reportRejection(handleSave())}`, never `onClick={handleSave}` with an async `handleSave`, which drops the rejection on the floor. A bare `void x()` is right only where the callee already logs its own failures and resolves either way — `getConsentState`, `getAllPresets`, `setupUninstallUrl` — and each of those sites says so in a comment. `trackEvent` returns `void` for the same reason: `captureEvent` behind it swallows and logs everything, and only the tests need the promise.
