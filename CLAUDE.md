...
- **E2E tests**: Playwright (`playwright.config.ts`, `tests/e2e/`). Tests load the test build (`pnpm build:test` first) as an unpacked extension and drive the service worker + extension pages. Use the `TestHelpers` and fixtures in `tests/e2e/fixtures.ts`. `chromeExtensionId` is read from `package.json`. CI uses 4 workers, matching the runner's vCPUs; the suite does not scale past that, so `fullyParallel` stays off. Reporting stays on the built-in `github` reporter, which annotates every failed attempt including retried-and-passed ones - flaky tests are meant to be visible, not filtered out.
- **The E2E browser is shared per worker.** The `context` fixture launches one persistent context per Playwright worker and resets the extension between tests (`resetExtensionState` in `tests/e2e/fixtures.ts`: clears all three storage areas, closes the tabs and the side panel, drops any `context.route`). Assume nothing about state a previous test left, and if a test needs a browser the extension has never run in — anything that depends on `chrome.runtime.onInstalled`, which fires once per profile — put `test.use({ shouldIsolateBrowser: true })` at the top of the spec file, as `initialization.spec.ts` does. That opt-out costs a browser launch per test, so keep it to the specs that genuinely need it.
