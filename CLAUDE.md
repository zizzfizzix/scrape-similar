...
- **`no-misused-promises` runs with `checksVoidReturn.attributes` off, and only that.** A React handler prop is `() => void` because React has no other spelling for one, so `onClick={async () => …}` is the idiomatic form and the rewrite the rule wants (`onClick={() => void handle()}`) discards the same promise with more syntax. Everywhere else a void return is expected the rule stays on — `browser.*.addListener`, `storage.watch`, `addEventListener` — because a rejection there has nowhere to land.
