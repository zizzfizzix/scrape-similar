...
- **Unit-test harness**: **`@testing-library/react`**, with `user-event` for interactions and `jest-dom`'s matchers (registered for every file in `vitest.setup.ts`). The hand-rolled `tests/support/react.tsx` it replaced is gone; nothing should reintroduce a local `render` — a _stand-in_ for Testing Library's, that is. `renderSettled` below is not one: it calls `render` and hands back its `RenderResult`. Four things the migration in #266 had to settle, all of which will come up again:
  - **`render` is synchronous.** The old harness awaited `act`, so a component that reads storage on mount had settled by the first assertion. Prefer `await screen.findBy*` / `await waitFor(...)`; where a whole file asserts against a mount-time read, its local `render` helper mounts through **`renderSettled`** (`tests/support/settle.ts`), which is Testing Library's own `render` plus one flush. Each file still writes the helper that knows which providers its component needs — what it no longer writes is the flush. **`settleEffects`** from the same module is that flush on its own, for letting a storage write's watchers land mid-test.
  - **Never nest `act` around `user-event`** — or around `fireEvent`, which also wraps itself. `act(() => userEvent.click(x))` returns a promise from a sync callback and deadlocks the test. `testing-library/no-unnecessary-act` enforces this since [#280](https://github.com/zizzfizzix/scrape-similar/issues/280); the one `act` it cannot judge is the empty flush inside `settleEffects`, disabled on that line with the reason. Do not add a second disable — call `settleEffects`.
...
- **jsdom shims** live in `vitest.setup.ts` (`matchMedia`, `PointerEvent`, pointer capture, `scrollIntoView`, `scrollTo`, `ResizeObserver`) — everything Radix and the layout-measuring components reach for. `tests/support/dom.ts` holds the one shim a test has to opt into with a value of its own, `stubOffsetWidth`. `tests/support/**` is linted as test code, so the harness modules answer to the same rules as their callers.
...
- **`DEFERRED` is the worklist, not a dumping ground.** Each rule in it is off with the number of violations it reports and why they are not a diff away — `no-floating-promises` (128, of which 65 are `trackEvent`), `no-misused-promises` (34), `no-explicit-any` (72). Re-enabling one means fixing what it names; the counts are there to be checked, so update them if they move. Never add a rule to `DEFERRED` to make a check pass without recording the same two things. The four React Compiler rules that wanted effects restructured came out in [#279](https://github.com/zizzfizzix/scrape-similar/issues/279), so the whole `react-hooks` set is on: an effect that mirrors a prop or an external source into state is a lint error now, not a review comment.
- **`OFF_ON_MERITS` is the other list, and it is not a worklist.** A rule lands there when it is wrong for this codebase, so re-reading it needs a reason that has changed rather than an afternoon. Two `jest-dom` rules: `prefer-to-have-text-content` and `prefer-to-have-style` rewrite exact assertions into weaker ones (substring matching; no way to say a declaration is unset), and both fixers mangle `expect((el as HTMLElement).…)` into unparseable code. And `testing-library/no-node-access`, which [#280](https://github.com/zizzfizzix/scrape-similar/issues/280) moved out of `DEFERRED` after answering the question behind its 268 sites: 78 of them select on something Testing Library has no query for by design — a computed `className`, a `lucide-*` icon, `cmdk` / `vaul` / `data-slot` internals, the component's own root, an inline `style`, `input[type="file"]` — so the suite cannot be query-first and the rule could never be switched on. Reaching into `container` is the deliberate choice here; prefer a query where one exists anyway.
