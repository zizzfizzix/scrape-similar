...
- **`naming-convention` is on for one row of the conventions, not all of them.** Booleans only (#284); the `MESSAGE_TYPES`-style constant and directory/function casing rows have never produced a review comment, and the rule would mostly add opinions about them that nobody asked for. Its two sharp edges are recorded beside it: it strips `null | undefined` from a type before checking (`getNonNullableType()`), so it cannot tell a tri-state from a boolean, and it is scoped to `variable` rather than every declaration.
